...
admx.col.lst <- admx.col.lst[names(admx.col.lst) != 'asia']
...
bio11.plt <- ggarrange(plotlist = bio11.plt.lst, nrow = 2, ncol = 2, common.legend = T, legend = 'bottom')

ggsave(bio11.plt, filename = '../figures/growthParametersVSbio11.pdf', width = 10.5, height = 10.5)
```

```{r growth parameters vs bio11 - exclude Swedish accessions}
# bio11 correlation plots
bio11.plt.lst <- map(phenotypes, function(pheno){
  phenoClim %>%
    select(accession, !!pheno, bio_11) %>%
...
bio11.exclSWE.plt <- ggarrange(plotlist = bio11.plt.lst, nrow = 2, ncol = 2, common.legend = T, legend = 'bottom')
ggsave(bio11.exclSWE.plt, filename = '../figures/growthParametersVSbio11.excludeSWE.pdf', width = 10.5, height = 10.5)
```
...
bio11.onlySWE.plt <- ggarrange(plotlist = bio11.plt.lst, nrow = 2, ncol = 2, common.legend = T, legend = 'bottom')
ggsave(bio11.onlySWE.plt, filename = '../figures/growthParametersVSbio11.onlySWE.pdf', width = 10.5, height = 10.5)
```

```{r coefficient of varaince of each growth parametr vs bio11}
coefVar<- function(x){
  cv <- sd(x)/mean(x)
  return(cv)}
...
bio11.plt.lst <- map(phenotypes, function(pheno){
  pheno.CV <- phenoClim %>%
    select(accession, !!pheno) %>%
    rename('phenotype' = !!pheno) %>%
    left_join(select(meta, accession, group), by = 'accession') %>%
    drop_na() %>%
    group_by(group) %>%
    summarise(pheno.CV = coefVar(phenotype))
  ...
  pheno.CV %>%
    ...
    ggplot(aes(x = bio11.median, y = pheno.CV)) +
    geom_point(aes(color = group)) +
    scale_color_manual(values = admx.col.lst) +
    geom_smooth(method = 'lm', se = F, color = 'grey') +
    stat_cor(method = 'pearson', size = 5, label.x.npc = 0, label.y.npc = 1) +
...
bio11.CV.plt <- ggarrange(plotlist = bio11.plt.lst, nrow = 2, ncol = 2, common.legend = T, legend = 'bottom')
ggsave(bio11.CV.plt, filename = '../figures/growthParametersCoefVarVSbio11.pdf', width = 10.5, height = 10.5)
```

```{r coefficient of varaince of each growth parametr vs bio11 - exclude asian accessions}
coefVar<- function(x){
  cv <- sd(x)/mean(x)
  return(cv)}
...
bio11.CV.exclAsian.plt <- ggarrange(plotlist = bio11.plt.lst, nrow = 2, ncol = 2, common.legend = T, legend = 'bottom')
ggsave(bio11.CV.exclAsian.plt, filename = '../figures/growthParametersCoefVarVSbio11.excludeAsian.pdf', width = 10.5, height = 10.5)
```
...
bio11.exclASIA.plt <- ggarrange(plotlist = bio11.plt.lst, nrow = 2, ncol = 2, common.legend = T, legend = 'bottom')
ggsave(bio11.exclASIA.plt, filename = '../figures/growthParametersVSbio11.excludeASIA.pdf', width = 10.5, height = 10.5)
```

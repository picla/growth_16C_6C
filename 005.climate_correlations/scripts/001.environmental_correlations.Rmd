...
bio11.plt <- ggarrange(plotlist = bio11.plt.lst, nrow = 2, ncol = 2, common.legend = T, legend = 'bottom', labels = 'AUTO')
...
ggsave(bio11_r_resp.exclSWE.plt, filename = '../figures/r_responseVSbio11.excludeSWE.pdf', width = 10.5, height = 10.5)
```
...
ggsave(bio11_r_resp.onlySWE.plt, filename = '../figures/r_responseVSbio11.onlySWE.pdf', width = 10.5, height = 10.5)
```

```{r combine figures of only Swedish and without Swedisch accessions}
bio11_r_resp.comboSWE.plt <- ggarrange(bio11_r_resp.onlylSWE.plt +
                                           scale_color_manual(values = admx.col.lst,
                                                              labels = admx.labels.lst),
                                       bio11_r_resp.exclSWE.plt +
                                           scale_color_manual(values = admx.col.lst,
                                                              labels = admx.labels.lst),
                                       labels = 'AUTO', ncol = 2, nrow = 1, common.legend = T, legend = 'bottom')
ggsave(bio11_r_resp.comboSWE.plt, filename = '../figures/r_responseVSbio11.comboSWE.pdf', width = 10.5, height = 5.25)
```
...
r_response_bio11.CV.plt <- r_response.CV %>%
    ...
    scale_color_manual(values = admx.col.lst,
                       labels = admx.labels.lst) +
    geom_smooth(method = 'lm', se = F, color = 'grey') +
    stat_cor(method = 'pearson', size = 5, label.x.npc = 0.82, label.y.npc = 'top') +
    theme_classic() +
...
ggsave(r_response_bio11.CV.plt, filename = '../figures/r_responseCoefVarVSbio11.pdf', width = 10.5, height = 10.5)
```

---
title: "002.adaptive_differentiation_exclAsian"
...
admx.labels.lst <- admx.labels.lst[names(admx.labels.lst) != 'asia']
...
```{r genotypes}
# filter out Asian accessions to check their influence
accessions.asia <- filter(meta, group == 'asia') %>% pull(accession)
growth <- growth %>%
  filter(!(accession %in% accessions.asia))
...
quaint.plt.lst <- map(phenotypes, function(pheno.name)
{
  ...
  pheno.plt.lst <- map(PC.sign, function(PC.nr){
    ...
    PC.pheno.plt.admx <- ggplot(PC.pheno.plt.tbl, aes(x = PC, y = pheno)) +
      ...
      scale_color_manual(values = admx.col.lst,
                         labels = admx.labels.lst) +
      labs(x = K.eigen.percs.lab[PC.nr], y = pheno.labels[pheno.name], color = 'subpopulation') +
      theme_classic()
...
write_csv(Qpc.pvals, file = '../results/quaint_pvalues_excludeAsia.csv')
```


```{r plot M0 vs PC6, r_16C vs PC5 r_response vs PC5 in combined figure, message = F}
M0_PC1.plt <- quaint.plt.lst$M0$PC1 +
  theme(text = element_text(size = 24))

M0_PC5.plt <- quaint.plt.lst$M0$PC5 +
  theme(text = element_text(size = 24))

M0_PC9.plt <- quaint.plt.lst$M0$PC9 +
  theme(text = element_text(size = 24))
...
combo.plt <- ggarrange(M0_PC1.plt, M0_PC5.plt, M0_PC9.plt, r_16C_PC5.plt, r_response_PC5.plt, common.legend = T, legend = 'bottom', ncol = 3, nrow = 2, labels = 'AUTO', font.label = list(size = 24))


ggsave(filename = '../figures/quaint_M0_PC1_M0_PC5_M0_PC9_r_16C_resp_PC5_excludeAsia.pdf',
       plot = combo.plt,
       width = 21, height = 14)
```
